/**
 * Provides the default behaviours for the frontend editor, including bells and whistles for
 * image upload selection and live word/character counts, etc.
 */
jQuery(document).ready(function($) {
	// Much of the frontend editor JS depends on the availability of the directorium.listing
	// object, so check that it is available first of all
	if (typeof directorium !== "undefined" && typeof directorium.listing !== "undefined") {
		...
		// Total the number of attached images less the ones marked for deletion
		...
		// Total the number of image upload inputs (not necessarily those which contain a file selection)
		function countImageInputs() {
			return $(".imageuploadinputs input[type='file']").length;
		}

		// True/false: should we allow a new image input to be created?
